% ===================== 中文教学注释 =====================
% 文件路径: Conductor Decomposition/assemble_IntermeshBoundaryFluxMatrix_2.m
% 教学提示: 请结合 docs/教学导读.md 中相应章节阅读本文件，以理解其作用和关键算法。
% ===================== 中文教学注释 =====================
function S = assemble_IntermeshBoundaryFluxMatrix_2(msh_master, msh_slave, boundaryData, v_master, S)
...
elements_slave = msh_slave.e2t(1, boundaryData(6,:));
...
[F_slave, F0_slave] = mappingTerms(msh_slave, elements_slave); detF_slave = mappingDeterminant(F_slave);
...
%
nref = [0 -1;1 1;-1 0]';
[dotProduct(ne_slave, mappingTimesVector(nref(:,1), 0,0,F_slave)); 
    dotProduct(ne_slave, mappingTimesVector(nref(:,2), 0,0,F_slave)); 
    dotProduct(ne_slave, mappingTimesVector(nref(:,3), 0,0,F_slave))]
...
phi_ref_bnd = { [-1 1]; [1 0] };
fh_master = @(ind, X)( phi_ref_bnd{ind} * X );

NO_f1 = 2; NO_f2 = 3;
%INDM_1 = msh_master.t(:, boundaryData(1,:));
INDM_1 = msh_master.edges(:, boundaryData(1,:));
INDM_2 = msh_slave.t(:, elements_slave);
