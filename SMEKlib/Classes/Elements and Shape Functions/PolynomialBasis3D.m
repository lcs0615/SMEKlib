% ===================== 中文教学注释 =====================
% 文件路径: Classes/Elements and Shape Functions/PolynomialBasis3D.m
% 教学提示: 请结合 docs/教学导读.md 中相应章节阅读本文件，以理解其作用和关键算法。
% ===================== 中文教学注释 =====================
classdef PolynomialBasis3D < handle
    properties
    ...
    methods(Static)
        ...
        function [xorder, yorder, zorder] = getOrders(element, shapefunction)
            [xorder, yorder, zorder] = PolynomialBasis3D.getOrdersFromBT( ...
                PolynomialBasis3D.getBasisType(element, shapefunction) );
        end
           
        
        function v = eseval(X, element, shapefunction)
            [xorder, yorder, zorder] = PolynomialBasis3D.getOrders(element, shapefunction);            
            v = bsxfun(@power, X(1,:), xorder) .* ...
                bsxfun(@power, X(2,:), yorder) .* ...
                bsxfun(@power, X(3,:), zorder);
        end
        function v = bteval(X, bt)
            [xorder, yorder, zorder] = PolynomialBasis3D.getOrdersFromBT(bt);
            v = bsxfun(@power, X(1,:), xorder) .* ...
                bsxfun(@power, X(2,:), yorder) .* ...
                bsxfun(@power, X(3,:), zorder);
        end
        ...
        function C2 = curl_coeffs(C, varargin)
            if numel(varargin) == 1
                ...
                bt = PolynomialBasis3D.getBasisType(varargin{1}, varargin{2});
            end
            [xorder, yorder, zorder] = PolynomialBasis3D.getOrdersFromBT(bt);
            
            [dxM, dyM, dzM] = PolynomialBasis3D.derivated_coeffs(xorder, yorder, zorder);
    ...
    %non-static methods
    methods
        function Pb = PolynomialBasis3D(varargin)
            if numel(varargin) == 1
                bt = varargin{1};
            else
                bt = PolynomialBasis3D.getBasisType(varargin{1}, varargin{2});
