% ===================== 中文教学注释 =====================
% 文件路径: Classes/Mesh/compute_SurfaceForce.m
% 教学提示: 请结合 docs/教学导读.md 中相应章节阅读本文件，以理解其作用和关键算法。
% ===================== 中文教学注释 =====================
function F = compute_SurfaceForce(Br, msh1_top, msh1_bottom, msh2_top, msh2_bottom)
...
for kq = 1:numel(wq)
    ...
    for kq2 = 1:numel(wq)
        ...
        for ke2 = 1:size(msh2_top.t, 2)
            ...
            dx = x1t - x2t(:, ke2);
            Hs_top = Hs_top + dx ./ sum(dx.^2, 1).^1.5 * det2t(ke2)*wq(kq2);

            dx = x1t - x2b(:, ke2); 
            Hs_top = Hs_top - dx ./ sum(dx.^2, 1).^1.5 * det2b(ke2)*wq(kq2);

            dx = x1b - x2t(:, ke2); 
            Hs_bottom = Hs_bottom - dx ./ sum(dx.^2, 1).^1.5 * det2t(ke2)*wq(kq2);

            dx = x1b - x2b(:, ke2); 
            Hs_bottom = Hs_bottom + dx ./ sum(dx.^2, 1).^1.5 * det2b(ke2)*wq(kq2);
        end
    end
